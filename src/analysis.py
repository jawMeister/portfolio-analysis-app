...
def plot_density_plots(results):
    ...
    with st.spinner('Calculating Probability Density Plots...'):
        ...
        with col1:
            _, _, fig1 = plot_probability_density_for_a_given_year("Year 1", [result.iloc[1].sum() for result in results])
            st.plotly_chart(fig1)
            
        with col2:
            _, _, fig2 = plot_probability_density_for_a_given_year(f'Year {N//2}', [result.iloc[mid_index].sum() for result in results])
            st.plotly_chart(fig2)
            
        with col3:
            _, _, fig3 = plot_probability_density_for_a_given_year(f'Year {N}', [result.iloc[-1].sum() for result in results])
            st.plotly_chart(fig3)
        ...
        with stqdm(total=len(years_to_plot)) as pbar:
            with concurrent.futures.ProcessPoolExecutor(max_workers=min(len(years_to_plot), n_cores)) as executor:
                ...
                for year in years_to_plot:
                    ...
                    future = executor.submit(plot_probability_density_for_a_given_year, f"Year {year}", values)
                    futures.append(future)
